...
RUN  apt install -y screen  nano fonts-noto-cjk && rm -rf /var/lib/apt/lists/*
WORKDIR /app
...
COPY ./start.sh /app/
RUN chmod -R 755 /app&& chmod 755 /usr/bin/mp4decrypt &&chmod 755 /usr/bin/ttyd &&chmod 755 /app/start.sh
